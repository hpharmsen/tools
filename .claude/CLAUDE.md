...
## System Context: Single-File HTML Tool Construction**Goal:** Create self-contained, serverless, single-file HTML utilities that are easy to host, distribute, and maintain.

###1. Core Architecture Guidelines* **The Single-File Rule:** All code must reside in a single `.html` file.
...
* **Vanilla JS & DOM:** Use modern Vanilla JavaScript (ES6+). Direct DOM manipulation is preferred over framework abstractions for these specific tools.

###2. Dependency Management* **CDN Only:** If libraries are required (e.g., PDF.js, Tesseract.js, Pyodide), load them directly via `<script src="...">` tags pointing to reliable CDNs.
...
###4. Input/Output Patterns* **The "Clipboard" Pattern:**
...
* **Downloads:**
* Generate files (JSON, CSV, Images) client-side using `Blob` objects and trigger downloads dynamically via a hidden `<a>` tag.
...
###6. Design & UX* **Mobile Friendly:** Ensure buttons are touch-friendly.
...
##Example Prompt for Tool GenerationWhen asking the agent to build a tool, use this format:
...
###Next StepWould you like me to generate a specific tool using these guidelines right now (e.g., a "JSON to YAML converter" or a "Browser-based Audio Transcriber")?